...
hardcoded to ``.hermes``.  Keep this file in sync with the hermes original when
picking up upstream storage fixes.
...
class FileSync:
    ...
    def __init__(
        ...
        self._exclude_dirs = {
            ...
            ".local",
            ".mc",
            # The runtime home is Worker-local; RemoteWorker pushes a curated
            # subset of it separately.
            runtime_home_dir,
            "platforms",
            "matrix-nio-store",
            "image_cache",
    ...
    def push_local(self, since: float = 0) -> list[str]:
        ...
        Excludes Manager-managed files, caches, and the whole runtime-home dir
        (``self._runtime_home_dir``). AGENTS.md, SOUL.md and memory/ are
        Worker-managed and are pushed.
        """
        _EXCLUDE_FILES = {
            "openclaw.json",
            "mcporter-servers.json",
        ...
        # Files directly inside the runtime home that bridge.py derives from
        # openclaw.json — pushing them back would revert Manager-driven edits.
        _RUNTIME_DERIVED_FILES = {
            "config.json",
            ".env",
        }
        ...
        for path in local_dir.rglob("*"):
            ...
            if rel.as_posix() in _EXCLUDE_PATHS:
                continue
            if any(p in self._exclude_dirs for p in rel.parts):
                continue
            if rel.suffix in _EXCLUDE_EXTENSIONS:
                continue
            if (
                len(rel.parts) == 2
                and rel.parts[0] == self._runtime_home_dir
                and rel.name in _RUNTIME_DERIVED_FILES
            ):
                continue

            key = f"{self._prefix}/{rel.as_posix()}"
            try:
                remote = self._cat(key)
...
def push_local(sync: FileSync, since: float = 0) -> list[str]:
    """Module-level wrapper kept for call-site compatibility.

    Prefer :meth:`FileSync.push_local`.
    """
