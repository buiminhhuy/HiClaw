...
func (r *HumanReconciler) reconcileHumanRooms(ctx context.Context, s *humanScope) {
	...
	//
	// Unlike additions which rely on status.RoomID/TeamRoomID, removals are
	// based on the spec: we only kick if the team/worker was explicitly
	// removed from accessibleTeams/AccessibleWorkers. This prevents
	// phantom removals when a team hasn't finished provisioning yet
	// (TeamRoomID is empty) or when status hasn't been updated.
	kept := next[:0]
	for _, rid := range next {
		...
		if workerName := findWorkerNameByRoomID(ctx, r.Client, h.Namespace, rid); workerName != "" {
			for _, accessibleWorker := range h.Spec.AccessibleWorkers {
				if accessibleWorker == workerName {
					// Worker is still accessible, don't kick even though status.RoomID might be empty
					kept = append(kept, rid)
					break
				}
			}
		...
		// Not in desired, and not in accessibleTeams/Workers - safe to kick
